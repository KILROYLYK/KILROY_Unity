Shader "_Custom/Water/WaterX"
{
    ...
    SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutput o)
        {
            ...
            fixed4 foam = (tex2D(_FoamTex, foamUV1) + tex2D(_FoamTex, foamUV2)) *
                lerp(_FoamColor, _ShadowColor, shadow);
            float3 rippleNormals = normalize(lerp(UnpackNormal(tex2D(_Ripples, foamUV1)),
                                                  UnpackNormal(tex2D(_Ripples, foamUV2)), noise));
